...
<html>
	...
	<body>
		...
		<div class="container">
	        ...
	        <hr/>
	        <div class="reply"> <%-- 댓글 작성 양식(form) --%>
	        	<textarea rows="3" id="rtext" name="rtext" placeholder="댓글 입력" class="DOC_TEXT" ></textarea> 
	            <br/>
	            
	            <%-- 로그인한 사용자 아이디를 input의 값으로 설정 --%>
	            <input type="hidden" id="reply_userid" name="userid" value="${signInUserId}" readonly />
	                ...
	                $.getJSON('/ex02/replies/all/' + boardNo, function (respText) {
	                    ...
	                    $(respText).each(function () {
	                    	...
	                    	list += '<div class="reply_item">'
	                    		   ...
	                    		   + this.rno
	                    		   + '" readonly />'
	                    		   + '<textarea rows="2" id="rtext" name="rtext">' 
	                    		   + this.rtext
	                    		   + '</textarea><br/>'
	                    	...
	                    	if (this.userid == '${signInUserId}') { // 댓글 작성자 아이디와 로그인한 사용자 아이디가 같으면
	                    		list += '<button class="reply_update">수정</button>'
	                    			  + '<button class="reply_delete"style="margin-right: 5px;">삭제</button>';
	                    	}
	                    	list += '</div><hr/>';
	                    });
